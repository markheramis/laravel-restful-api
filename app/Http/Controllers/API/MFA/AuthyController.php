...
class AuthyController extends Controller
{
     ...
     */
    protected function sendVerificationCodeSMS(Request $request, AuthyApi $authyApi)
    {
     ...
     * @return Illuminate\Support\Facades\Response;
     */
    protected function sendVerificationCodeVoice(Request $request, AuthyApi $authyApi)
    {
        $user_id = session('user_id');
        $user = User::find($user_id);
        $authyID = $user['authyID'];
     ...
     * @return Illuminate\Support\Facades\Response;
     */
    protected function verifyToken(Request $request, AuthyApi $authyApi)
    {
        $data = $request->all();
        $token = $data['token'];
        $user_id = session('user_id');
     ...
     * @return Illuminate\Support\Facades\Response;
     */
    protected function createOneTouch(Request $request, OneTouch $oneTouch)
    {
        $data = $request->all();
        $user_id = session('user_id');
        $user = User::find($user_id);
     ...
     * @return Illuminate\Support\Facades\Response;
     */
    protected function checkOneTouchStatus(Request $request, OneTouch $oneTouch)
    {
